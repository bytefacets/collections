...
class CompactOneToManyTest {
    private final CompactOneToMany collection = new CompactOneToMany(5, 5, true);
        ...
        IntStream.of(3, 4, 0, 2, 1)
                .forEach(
                        right -> {
                            ...
                            assertThat(collection.size(), equalTo(collection.size()));
                            assertThat(collection.contains(2, right), equalTo(false));
    ...
    class RemoveAndReserveTests {
        ...
        void shouldNotAddEntryToFreeListBeforeExplicitlyDone() {
            assertThat(collection.put(2, 7), equalTo(5)); // doesn't reuse 3
